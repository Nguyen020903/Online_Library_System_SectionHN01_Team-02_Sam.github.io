  ...
  if (user) {
    try {
      ...
      res
        .status(500)
        .json({
          error: "An error occurred while fetching the user's favorite books",
        });
    }
  }
...
module.exports.add_to_wishlist_post = async (req, res) => {
  ...
  if (user) {
    try {
      ...
      res
        .status(500)
        .json({ error: "An error occurred while adding the book to the cart" });
    }
  }
  ...
  if (user) {
    try {
      ...
      res
        .status(500)
        .json({
          error: "An error occurred while removing the book from the wishlist",
        });
    }
  } else {
            ...
            transaction._id,
            {
              $set: {
                ...
                fine:
                  1000 *
                  Math.floor(
                    (Date.now() - new Date(transaction.returnDate)) /
                      (1000 * 60 * 60 * 24)
                  ),
              },
            },
...
module.exports.reservations_return_post = async (req, res) => {
  try {
    ...
    const user = await User.findByIdAndUpdate(
      transaction.userId,
      { $pull: { activeTransactions: id }, $push: { prevTransactions: id } },
      { new: true }
    );
    if (!user) {

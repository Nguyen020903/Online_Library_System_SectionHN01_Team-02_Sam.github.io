...
const bookImageStorage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    // 'file' and 'cb' parameters were swapped
...
router.post(
  ...
  checkUser,
  isAdmin,
  multer({ storage: bookImageStorage }).single("bookImage"),
  bookController.addbook_post
);
...
router.get(
  ...
  checkUser,
  isAdmin,
  bookController.updatebook_get
);
router.post(
  "/updateBook/:id",
  checkUser,
  isAdmin,
  bookImageUpload.single("bookImage"),
  bookController.updatebook_post
);
